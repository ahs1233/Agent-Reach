...
def _research_case(number):
    ...
    if number == 18:
        ...
        return "Evidence linked to source", {
            "run_id": run["run_id"],
            "evidence_id": evidence["evidence_id"],
        }
    if number == 19:
        run, _, evidence, claim = _seed(store, slug="claim")
        assert evidence["evidence_id"] in [
            x["evidence_id"] for x in store.export_run(run["run_id"])["evidence"]
        ]
    ...
    if number == 27:
        ...
        return "Source version chain preserved", {
            "v1": first["source_id"],
            "v2": second["source_id"],
        }
    if number in {28, 29, 34, 35}:
        ...
        for idx, pub in enumerate(("Publisher A", "Publisher B")):
            source = store.record_source(
                run["run_id"],
                url=f"https://lineage-{idx}.example.com/x",
            ...
            ev = store.add_evidence(
                run["run_id"],
                source_id=source["source_id"],
                supporting_passage=f"support {idx}",
        ...
        if number in {29, 35}:
            rel = store.record_source_relationship(
                ...
                basis="independent collection verified",
            )
            ...
            rel = store.record_source_relationship(
                ...
                basis="same representation",
            )
            assert rel["relationship_type"] == "MIRRORS"
        claim = store.add_claim(
            run["run_id"],
            statement="lineage claim",
            classification="VERIFIED",
            observation_type="ACTUAL",
    ...
    if number == 33:
        result = evaluate_freshness(
            data_cutoff="2026-09-23T09:00:00Z",
...
def _runtime_case(number, tmp_path):
                    ...
                    "tasks": [
                        {
                            ...
                            "objective": "a",
                            "steps": [
                                {"id": "d", "tool_name": "reach_doctor", "role": "orchestrator"}
                            ],
    ...
    if number == 56:
        _payload(
            gateway.call_tool(
                "runtime_memory_put", {"key": "golden-memory", "content": "golden durable value"}
            )
        )
    ...
    if number in {58, 60}:
        ...
        result = _payload(
            gateway.call_tool("runtime_skill_execute", {"name": f"golden-skill-{number}"})
        )
        assert result["status"] == "ok"
        if number == 60:
            skill = _payload(
                gateway.call_tool("runtime_skill_get", {"name": f"golden-skill-{number}"})
            )
            assert skill["successes"] >= 1 and skill["score"] > 0.5
            return "Skill score updated from outcome", {
                "score": skill["score"],
    ...
    if number == 59:
        ...
        result = _payload(
            gateway.call_tool("runtime_skill_execute", {"name": "golden-failing-skill"})
        )
        ...
        return "Failed skill recorded failure honestly", {
    ...
    if number == 61:
        _payload(
            gateway.call_tool(
                "runtime_skill_save",
                {
                    "name": "golden-rollback",
                    "workflow": [{"id": "a", "tool_name": "reach_doctor"}],
                    "change_note": "v1",
                },
            )
        )
        _payload(
            gateway.call_tool(
                ...
                {
                    "name": "golden-rollback",
                    "workflow": [{"id": "b", "tool_name": "reach_doctor"}],
                    "change_note": "v2",
                },
            )
        )
        rolled = _payload(
            gateway.call_tool("runtime_skill_rollback", {"name": "golden-rollback", "revision": 1})
        )
    ...
    if number == 62:
        started = _payload(
            gateway.call_tool(
                "orchestration_start",
                {
                    "objective": "golden research routing",
                    "mode": "research",
...
def _retrieval_case(number):
    ...
    def caller(name, arguments):
        ...
        if number in {4, 66}:
            return (
                _err("Jina unavailable") if name == "reach_read_url" else _ok("usable " + "x" * 400)
            )
        if number == 5:
        ...
        if number == 6:
            return (
                _ok("browser " + "x" * 400)
                if name == "golden_browser"
                else _err("backend unavailable")
            )
        if number == 7:
            return _err("backend unavailable")
    ...
    return "Fallback state machine behaved deterministically", {
...
def _http_server(gateway, token=""):
    handler = type(
        "GoldenHandler", (ToolboxRequestHandler,), {"gateway": gateway, "auth_token": token}
    )
...
def _failure_case(number, tmp_path):
    ...
    if number in {67, 75}:
        ...
        if number == 75:
            ...
            assert doctor
            return "Remote dependency failure did not kill local core", {
                "local_tools": len(names),
                "doctor": doctor,
            }
    ...
    if number == 68:
        ...
        parent = _payload(
            gateway.call_tool(
                "orchestration_start",
                {
                    "objective": "golden failing child",
                    "mode": "general",
                    "budget": {"tool_calls": 2, "network_calls": 0},
                },
            )
                    ...
                    "tasks": [
                        {
                            ...
                            "steps": [
                                {
                                    "id": "x",
                                    "tool_name": "golden_nonexistent_tool",
                                    "role": "orchestrator",
                                }
                            ],
                        }
        ...
        return "Subagent/delegated failure remained isolated", {
    ...
    if number == 70:
        ...
        assert (
            second.get_source(source["source_id"], run_id=run["run_id"])["source_id"]
            == source["source_id"]
        )
...
def _integration_case(number, tmp_path):
    if number == 78:
        ...
        return "ACE research recorded Research Engine provenance", {
    ...
    if number == 80:
        ...
        source = store.record_source(
            ...
            content="Observed fact is 42.",
            retrieval_tool="fixture-search+read",
            retrieval_method="deterministic",
            retrieval_status="SUCCESS",
            retrieval_history=[
                {
        ...
        evidence = store.add_evidence(
            run["run_id"],
            source_id=source["source_id"],
            supporting_passage="Observed fact is 42.",
            observation_type="ACTUAL",
            value=42,
        )
        claim = store.add_claim(
            run["run_id"],
            statement="Observed fact is 42.",
            classification="VERIFIED",
            observation_type="ACTUAL",
            supporting_evidence_ids=[evidence["evidence_id"]],
        )
...
def _execute(case, tmp_path):
    ...
    if n == 14:
        parsed = feedparser.parse(
            b"""<?xml version="1.0"?><rss version="2.0"><channel><title>Golden</title><item><title>One</title><link>https://example.com/1</link></item></channel></rss>"""
        )
...
def test_golden_manifest_is_exact_and_complete():
    ...
    for item in CASES:
        assert {"id", "category", "preconditions", "action", "expected", "evidence", "tier"} <= set(
            item
        )
